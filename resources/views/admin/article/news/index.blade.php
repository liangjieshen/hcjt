            ...
            <div class="layui-col-md12">
                <div class="layui-card">
                    <div class="layui-card-body">
                        <form class="layui-form" method="get" action="{{ route('admin.article.newsList') }}">
                    ...
                    <div class="layui-card-header">
                        <button class="layui-btn" onclick="xadmin.open('添加相册','{{ route('admin.article.newsAdd') }}',900,580)"><i class="layui-icon"></i>添加新闻</button>
                    </div>
                        ...
                        <table class="layui-table layui-form">
                            ...
                            @foreach($list as $item)
                                <tr>
                                    ...
                                    <td><button class="layui-btn" onclick="xadmin.open('新闻内容','{{ route('admin.article.newsContent', ['id'=>$item->id]) }}',900,580)"><i class="layui-icon"></i>查看内容</button></td>
                                    <td>{{ $item->look }}次</td>
                                    <td>{{ date('Y-m-d H:i', $item->create_time) }}</td>
                                    <td>
                                        <button class="layui-btn layui-btn-warm layui-btn-sm"
                                                onclick="xadmin.open('编辑新闻','{{ route('admin.article.newsEdit', ['id'=>$item->id]) }}',900,580)">编辑
                                        </button>
...
@section('script')
    <script>
        ...
        layui.use(['element', 'form', 'layer', 'laydate'], function () {
            ...
            laydate.render({
                ...
                ,done: function(value, date, endDate){
                    ...
                    window.location.href = "{{ route('admin.article.newsList') }}?times="+value;
                }
            });
        ...
        function changeDelete(id) {
            ...
            layer.confirm('是否删除该新闻？',function(index){
                $.ajaxSetup({
                    headers: {
                ...
                var load = layer.load();
                $.post("{{ route('admin.article.newsDelete') }}", {id: id},
                    function (data) {
                        layer.close(load);
                        if (data.code === 0) {
                            layer.msg(data.msg, {
