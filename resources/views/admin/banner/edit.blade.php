...
@section('content')
    ...
    <form class="layui-form" action="">
        ...
        <div class="layui-form-item" style="width: 380px;">
            <label class="layui-form-label">所属导航名称</label>
            <div class="layui-input-block">
                <select name="nav_id" class="layui-select">
                    <option value="0">请选择所属导航名称</option>
                    @foreach($navList as $item)
                        <option value="{{ $item->id }}" @if($list->nav_id = $item->id)selected="selected"@endif>{{ $item->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            <label for="" class="layui-form-label">Banner封面</label>
            <div class="layui-input-block">
    ...
    <script>
        layui.use(['form', 'upload', 'layer'], function () {
            ...
            form.on('submit(formDemo)', function (data) {
                ...
                var load = layer.load();
                $.post("{{ route('admin.banner.save') }}", data.field,
                    function (data) {
                        layer.close(load);
                        if (data.code === 0) {
                            layer.msg('操作成功', {
