            ...
            <div class="layui-col-md12">
                <div class="layui-card">
                    <div class="layui-card-body">
                        <form class="layui-form" method="get" action="{{ route('admin.album.travelList') }}">
                    ...
                    <div class="layui-card-header">
                        <button class="layui-btn" onclick="xadmin.open('添加相册','{{ route('admin.album.travelAdd') }}',900,580)"><i class="layui-icon"></i>添加相册</button>
                    </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <button class="layui-btn layui-btn-warm layui-btn-sm"
                                                onclick="xadmin.open('编辑相册','{{ route('admin.album.travelEdit') }}?id={{ $item->id }}',900,580)">编辑
                                        </button>
...
@section('script')
    <script>
        ...
        layui.use(['element', 'form', 'layer', 'laydate'], function () {
            ...
            laydate.render({
                ...
                ,done: function(value, date, endDate){
                    ...
                    window.location.href = "{{ route('admin.album.travelList') }}?times="+value;
                }
            });
        ...
        $("._sort").change (function () {
            ...
            $.post("{{ route('admin.album.travelSort') }}", {id: id, sort: sort},
                function (data) {
                    if (data.code === 0) {
                        layer.msg(data.msg, {
                            offset: '15px'
        ...
        function changeState(id) {
            ...
            layer.confirm('是否更改该状态？',function(index){
                ...
                var load = layer.load();
                $.post("{{ route('admin.album.travelState') }}", {id: id},
                    function (data) {
                        layer.close(load);
                        if (data.code === 0) {
                            layer.msg(data.msg, {
        ...
        function changeDelete(id) {
            ...
            layer.confirm('是否删除该相册？',function(index){
                ...
                var load = layer.load();
                $.post("{{ route('admin.album.travelDelete') }}", {id: id},
                    function (data) {
                        layer.close(load);
                        if (data.code === 0) {
                            layer.msg(data.msg, {
